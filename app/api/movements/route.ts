...
export async function GET(req: Request) {
  ...
  return Response.json(await getDb().select().from(movements).where(inArray(movements.siloId, clientSilos.map(row => row.id))).orderBy(desc(movements.id)).limit(1000));
}
...
export async function POST(req: Request) {
  ...
  try {
    ...
    await getDb().insert(audits).values({ action: "INCLUSÃO", entity: "MOVIMENTAÇÃO", entityId: row.id, details: JSON.stringify({ type, qty: body.qty, siloId, adjustmentDelta: adjustmentDelta === null ? null : adjustmentDelta / 1000, doc: body.doc, scheduledLoadId }), userEmail: user.email, clientId, createdAt: new Date().toISOString() });
...
export async function DELETE(req: Request) {
  const access = await requireClient(req); if ("error" in access) return access.error;
  const { user, clientId } = access;
  if (user.role !== "admin") return Response.json({ error: "A exclusão é exclusiva do administrador." }, { status: 403 });
  ...
  const [silo] = await getDb().select().from(silos).where(and(eq(silos.id, row.siloId), eq(silos.clientId, clientId))).limit(1);
  if (!silo) return Response.json({ error: "Lançamento não pertence ao cliente selecionado." }, { status: 403 });
  ...
  await getDb().insert(audits).values({ action: "EXCLUSÃO", entity: "MOVIMENTAÇÃO", entityId: row.id, details: JSON.stringify({ date: row.date, type: row.type, qty: row.qty / 1000, siloId: row.siloId, doc: row.doc, vehicle: row.vehicle, scheduledLoadId: row.scheduledLoadId }), userEmail: user.email, clientId, createdAt: new Date().toISOString() });
