...
export async function GET(req: Request) {
  ...
  return Response.json(await getDb().select().from(silos).where(eq(silos.clientId, access.clientId)).orderBy(asc(silos.id)));
}

export async function POST(req: Request) {
  const access = await requireClient(req); if ("error" in access) return access.error;
  const { user, clientId } = access;
  if (user.role !== "admin") return Response.json({ error: "Cadastro de silos exclusivo do administrador." }, { status: 403 });
  ...
  const [row] = await getDb().insert(silos).values({ name, product, capacity, minimumStock, active: true, clientId, createdAt: now, updatedAt: now }).returning();
  await getDb().insert(audits).values({ action: "INCLUSÃO", entity: "SILO", entityId: row.id, details: JSON.stringify({ name, product, capacity: capacity / 1000, minimumStock: minimumStock / 1000 }), userEmail: user.email, clientId, createdAt: now });
...
export async function PATCH(req: Request) {
  const access = await requireClient(req); if ("error" in access) return access.error;
  const { user, clientId } = access;
  if (user.role !== "admin") return Response.json({ error: "Edição de silos exclusiva do administrador." }, { status: 403 });
  ...
  const [current] = await getDb().select().from(silos).where(and(eq(silos.id, id), eq(silos.clientId, clientId))).limit(1);
  ...
  await getDb().insert(audits).values({ action: "ALTERAÇÃO", entity: "SILO", entityId: id, details: JSON.stringify({ name, product, capacity: capacity / 1000, minimumStock: minimumStock / 1000, active }), userEmail: user.email, clientId, createdAt: updatedAt });
